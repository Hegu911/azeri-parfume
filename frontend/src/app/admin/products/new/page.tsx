...
export default function NewProduct() {
  ...
  const [form, setForm] = useState({
    ...
    categoryId: '', brandId: '', stock: '0', volume: '',
    topNotes: '', middleNotes: '', baseNotes: '',
    longevity: '', usageType: '', season: '',
    isFeatured: false, isBestSeller: false, isTrending: false, isActive: true,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/admin/products', fd, { headers: { Authorization: `Bearer ${localStorage.getItem('admin_token')}` } });
      router.push('/admin/products');
    } catch { alert('Error creating product'); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <h1 className="text-2xl font-light text-white">New Product</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white/5 rounded-2xl border border-white/5 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <textarea placeholder="Description" value={form.description} onChange={(e) => update('description', e.target.value)} rows={3} className="col-span-2 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
        ...
        <div className="bg-white/5 rounded-2xl border border-white/5 p-6 space-y-4">
          ...
          <div className="flex items-start gap-4">
            <div
              ...
              className="w-32 h-32 rounded-xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-[#d4af37] transition-colors bg-white/5"
            >
              {imagePreview ? (
        ...
        <div className="bg-white/5 rounded-2xl border border-white/5 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input type="number" step="0.01" placeholder="Cost Price" value={form.costPrice} onChange={(e) => update('costPrice', e.target.value)} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
            ...
            <input placeholder="Volume (e.g. 100ml)" value={form.volume} onChange={(e) => update('volume', e.target.value)} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
          </div>
        </div>
        ...
        <div className="bg-white/5 rounded-2xl border border-white/5 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <textarea placeholder="Top Notes" value={form.topNotes} onChange={(e) => update('topNotes', e.target.value)} rows={2} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
            <textarea placeholder="Middle Notes" value={form.middleNotes} onChange={(e) => update('middleNotes', e.target.value)} rows={2} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
            <textarea placeholder="Base Notes" value={form.baseNotes} onChange={(e) => update('baseNotes', e.target.value)} rows={2} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input placeholder="Longevity (e.g. 8-10 hours)" value={form.longevity} onChange={(e) => update('longevity', e.target.value)} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
            <select value={form.usageType} onChange={(e) => update('usageType', e.target.value)} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#d4af37]">
            ...
            <input placeholder="Season" value={form.season} onChange={(e) => update('season', e.target.value)} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#d4af37]" />
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <Button type="submit" variant="gold" loading={loading}>Create Product</Button>
